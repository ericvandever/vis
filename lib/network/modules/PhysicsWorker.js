...
class PhysicsWorker {
  ...
  initDatabase() {
    if (this.body.database === null) {
      console.log('initializing workers database');
      // TODO option for database name - default to undefined and disabled?
      //    let deleteRequest = indexedDB.deleteDatabase('visjs');

      let openRequest = indexedDB.open('visjs', 1);
      openRequest.onupgradeneeded = (e) => {
        ...
        this.body.database = db;
      };

      openRequest.onsuccess = (e) => {
        console.log("Success!");
        this.body.database = e.target.result;
      };
  ...
  handleMessage(event) {
    ...
    switch (msg.type) {
      ...
      case 'options':
        ...
        break;
      case 'physicsObjects':
        if (this.body.database) {
          console.log('reading from db');
          var transaction = this.body.database.transaction(["nodes"], "readonly");
          ...
          ob.onsuccess = function(e) {
            ...
            console.log('received values', cursor.value);
          }
        }
