...
class NodesHandler {
  ...
  setData(nodes, doNotEmit = false) {
    ...
    if (this.body.database) {
      let transaction = this.body.database.transaction(['nodes'], 'readwrite');
      let store = transaction.objectStore('nodes');
  ...
  create(properties, constructorClass = Node) {
    let node = new constructorClass(properties, this.body, this.images, this.groups, this.options);
    if (this.body.database) {
      let transaction = this.body.database.transaction(['nodes'], 'readwrite');
      let store = transaction.objectStore('nodes');
      let dbNode = {
        ...
        options: {
          ...
          physics: node.options.physics
        }
      };
      let request = store.add(dbNode);
      request.onerror = (e) => {
        console.log('error', e.target.error.name);
      };
    }
